...
/**
 * Naive representation of a P2 artifact unit
 */
 ...
 * }}}
 */
object P2Artifact extends FileNameVersionExtractor with LazyLogging {

  import io.coding.me.m2p2.core.internal.extension.StringExtensions._
 
  /**
   * Creates a list of P2 artifact representations based on a p2artfiacts.xml file.
   */
  def apply(file: File): Try[Option[Set[P2Artifact]]] = extractMavenVersion(file).flatMap { mavenVersion =>
      ...
      while (r.hasNext()) {
        ...
        event match {
          ...
          case s: EndElement if s.getName.getLocalPart == "artifact" =>
            ...
            //val mavenVersion = properties.get("maven-version") // Maven version points to SNAPSHOT literal, this is useless
            val mavenClassifier = properties.get("maven-classifier")
            val mavenExtension = properties.get("maven-extension")

            if (id.isDefined && version.isDefined && classifier.isDefined && mavenGroupId.isDefined && mavenArtifactId.isDefined) {

              val mavenGav = MavenGAV(mavenGroupId.get, mavenArtifactId.get, mavenVersion, mavenClassifier, mavenExtension)
